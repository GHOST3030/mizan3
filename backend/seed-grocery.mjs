...
async function main() {
  const company = await prisma.company.upsert({
    ...
    update: {},
    create: { id: '00000000-0000-0000-0000-000000000001', name: 'Mizan Store', name_ar: 'متجر ميزان' },
  });
  ...
  const branch = await prisma.branch.upsert({
    where: { id: BRANCH_ID },
    update: {},
    create: { id: BRANCH_ID, company_id: company.id, name: 'Main Branch', name_ar: 'الفرع الرئيسي', is_active: true },
  });
  ...
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: { branch_id: branch.id, name: 'المدير العام', username: 'admin', password_hash: hash, role: 'admin' },
  });
  console.log(' المستخدم:', admin.username);

  await prisma.currency.upsert({ where: { id: CURRENCY_YER }, update: {}, create: { id: CURRENCY_YER, code: 'YER', name: 'ريال يمني', symbol: '﷼', exchange_rate: 1, is_default: true } });
  await prisma.currency.upsert({ where: { id: '00000000-0000-0000-0000-000000000011' }, update: {}, create: { id: '00000000-0000-0000-0000-000000000011', code: 'USD', name: 'دولار أمريكي', symbol: '$', exchange_rate: 1500, is_default: false } });
  await prisma.currency.upsert({ where: { id: '00000000-0000-0000-0000-000000000012' }, update: {}, create: { id: '00000000-0000-0000-0000-000000000012', code: 'SAR', name: 'ريال سعودي', symbol: 'SR', exchange_rate: 400, is_default: false } });
  console.log(' العملات: YER, USD, SAR');

  const warehouse = await prisma.warehouse.upsert({ where: { id: WAREHOUSE_ID }, update: {}, create: { id: WAREHOUSE_ID, branch_id: BRANCH_ID, name: 'Main Warehouse', name_ar: 'المستودع الرئيسي', is_active: true } });
  console.log(' المستودع:', warehouse.name_ar);

  await prisma.cashRegister.upsert({ where: { id: '00000000-0000-0000-0000-000000000800' }, update: {}, create: { id: '00000000-0000-0000-0000-000000000800', branch_id: BRANCH_ID, name: 'الخزينة الرئيسية', balance: 500000, currency_id: CURRENCY_YER } });
  console.log(' الخزينة: الخزينة الرئيسية');

  const groups = {};
  for (const g of [{ id: '00000000-0000-0000-0000-000000000900', name: 'عادي' }, { id: '00000000-0000-0000-0000-000000000901', name: 'VIP' }, { id: '00000000-0000-0000-0000-000000000902', name: 'جملة' }]) {
    groups[g.id] = await prisma.customerGroup.upsert({ where: { id: g.id }, update: {}, create: g });
  }
  ...
  for (const c of customersData) {
    customers.push(await prisma.customer.upsert({ where: { id: c.id }, update: {}, create: { id: c.id, branch_id: BRANCH_ID, customer_group_id: c.group_id, name: c.name, phone: c.phone, balance: c.balance, credit_limit: c.credit_limit } }));
  }
  console.log(` العملاء: ${customers.length} عميل`);

  const supplierCats = {};
  for (const sc of [{ id: '00000000-0000-0000-0000-000000000a00', name: 'Food & Beverages', description: 'مواد غذائية ومشروبات' }, { id: '00000000-0000-0000-0000-000000000a01', name: 'Cleaning & Personal Care', description: 'منظفات وعناية شخصية' }, { id: '00000000-0000-0000-0000-000000000a02', name: 'Dairy & Fresh', description: 'ألبان ومواد طازجة' }]) {
    supplierCats[sc.id] = await prisma.supplierCategory.upsert({ where: { id: sc.id }, update: {}, create: sc });
  }
  console.log(' تصنيفات الموردين');
  ...
  for (const s of suppliersData) {
    suppliers.push(await prisma.supplier.upsert({ where: { id: s.id }, update: {}, create: { id: s.id, branch_id: BRANCH_ID, supplier_category_id: s.cat_id, name: s.name, phone: s.phone } }));
  }
  ...
  for (const u of unitsData) { units[u.id] = await prisma.unit.upsert({ where: { id: u.id }, update: {}, create: u }); }
  console.log(` الوحدات: ${unitsData.length}`);

  const brands = {};
  const brandsData = [
    { id: '00000000-0000-0000-0000-000000000300', name: 'Pepsi' }, { id: '00000000-0000-0000-0000-000000000301', name: 'Coca-Cola' },
    { id: '00000000-0000-0000-0000-000000000302', name: 'Hail' }, { id: '00000000-0000-0000-0000-000000000303', name: 'Almarai' },
    { id: '00000000-0000-0000-0000-000000000304', name: 'Al-Saeeda' }, { id: '00000000-0000-0000-0000-000000000305', name: 'Nido' },
    { id: '00000000-0000-0000-0000-000000000306', name: 'Lulu' }, { id: '00000000-0000-0000-0000-000000000307', name: 'Al-Watania' },
    { id: '00000000-0000-0000-0000-000000000308', name: 'Rabi' }, { id: '00000000-0000-0000-0000-000000000309', name: 'Nescafe' },
    { id: '00000000-0000-0000-0000-00000000030a', name: 'Lays' }, { id: '00000000-0000-0000-0000-00000000030b', name: 'Ulker' },
    { id: '00000000-0000-0000-0000-00000000030c', name: 'Tide' }, { id: '00000000-0000-0000-0000-00000000030d', name: 'Head & Shoulders' },
  ];
  for (const b of brandsData) { brands[b.id] = await prisma.brand.upsert({ where: { id: b.id }, update: {}, create: b }); }
  console.log(` العلامات: ${brandsData.length}`);
  ...
  for (const cat of categoriesData) { categories[cat.id] = await prisma.category.upsert({ where: { id: cat.id }, update: {}, create: cat }); }
  ...
  for (const p of productsData) {
    products.push(await prisma.product.upsert({ where: { id: p.id }, update: {}, create: { id: p.id, branch_id: BRANCH_ID, category_id: p.cat, unit_id: p.unit, brand_id: p.brand, name: p.name, name_ar: p.name_ar, barcode: p.barcode, cost_price: p.cost, selling_price: p.sell, min_stock: p.min, is_active: true } }));
  }
  console.log(` المنتجات: ${products.length} منتج`);

  const initialStock = [
    { id: '00000000-0000-0000-0000-00000000f001', qty: 120 }, { id: '00000000-0000-0000-0000-00000000f002', qty: 96 },
    { id: '00000000-0000-0000-0000-00000000f003', qty: 96 }, { id: '00000000-0000-0000-0000-00000000f004', qty: 72 },
    { id: '00000000-0000-0000-0000-00000000f005', qty: 72 }, { id: '00000000-0000-0000-0000-00000000f006', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f007', qty: 48 }, { id: '00000000-0000-0000-0000-00000000f008', qty: 18 },
    { id: '00000000-0000-0000-0000-00000000f009', qty: 60 }, { id: '00000000-0000-0000-0000-00000000f00a', qty: 24 },
    { id: '00000000-0000-0000-0000-00000000f00b', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f00c', qty: 25 },
    { id: '00000000-0000-0000-0000-00000000f00d', qty: 30 }, { id: '00000000-0000-0000-0000-00000000f00e', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f00f', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f010', qty: 20 },
    { id: '00000000-0000-0000-0000-00000000f011', qty: 72 }, { id: '00000000-0000-0000-0000-00000000f012', qty: 48 },
    { id: '00000000-0000-0000-0000-00000000f013', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f014', qty: 30 },
    { id: '00000000-0000-0000-0000-00000000f015', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f016', qty: 20 },
    { id: '00000000-0000-0000-0000-00000000f017', qty: 12 }, { id: '00000000-0000-0000-0000-00000000f018', qty: 96 },
    { id: '00000000-0000-0000-0000-00000000f019', qty: 72 }, { id: '00000000-0000-0000-0000-00000000f01a', qty: 48 },
    { id: '00000000-0000-0000-0000-00000000f01b', qty: 48 }, { id: '00000000-0000-0000-0000-00000000f01c', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f01d', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f01e', qty: 48 },
    { id: '00000000-0000-0000-0000-00000000f01f', qty: 72 }, { id: '00000000-0000-0000-0000-00000000f020', qty: 72 },
    { id: '00000000-0000-0000-0000-00000000f021', qty: 72 }, { id: '00000000-0000-0000-0000-00000000f022', qty: 72 },
    { id: '00000000-0000-0000-0000-00000000f023', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f024', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f025', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f026', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f027', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f028', qty: 12 },
    { id: '00000000-0000-0000-0000-00000000f029', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f02a', qty: 24 },
    { id: '00000000-0000-0000-0000-00000000f02b', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f02c', qty: 72 },
    { id: '00000000-0000-0000-0000-00000000f02d', qty: 24 }, { id: '00000000-0000-0000-0000-00000000f02e', qty: 48 },
    { id: '00000000-0000-0000-0000-00000000f02f', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f030', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f031', qty: 200 }, { id: '00000000-0000-0000-0000-00000000f032', qty: 60 },
    { id: '00000000-0000-0000-0000-00000000f033', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f034', qty: 36 },
    { id: '00000000-0000-0000-0000-00000000f035', qty: 36 }, { id: '00000000-0000-0000-0000-00000000f036', qty: 36 },
  ];

  for (const s of initialStock) {
    await prisma.inventoryBalance.upsert({
      where: { branch_id_warehouse_id_product_id: { branch_id: BRANCH_ID, warehouse_id: WAREHOUSE_ID, product_id: s.id } },
      update: { quantity: s.qty },
      create: { branch_id: BRANCH_ID, warehouse_id: WAREHOUSE_ID, product_id: s.id, quantity: s.qty },
    });
  }
  ...
  for (const s of initialStock) {
    ...
    await prisma.stockMovement.upsert({ where: { id: mid }, update: {}, create: { id: mid, branch_id: BRANCH_ID, warehouse_id: WAREHOUSE_ID, product_id: s.id, type: 'purchase', quantity: s.qty, notes: 'رصيد افتتاحي' } });
    moveIdx++;
  }
  ...
  for (const ec of expenseCatsData) { await prisma.expenseCategory.upsert({ where: { id: ec.id }, update: {}, create: { ...ec, branch_id: null } }); }
  console.log(` تصنيفات المصروفات: ${expenseCatsData.length}`);

  const roleDefs = [
    { name: 'super_admin', label: 'المشرف العام', is_system: true }, { name: 'admin', label: 'مدير النظام', is_system: true },
    { name: 'manager', label: 'مدير الفرع', is_system: true }, { name: 'accountant', label: 'محاسب', is_system: true },
    { name: 'inventory_manager', label: 'مسؤول المخزون', is_system: true }, { name: 'cashier', label: 'كاشير', is_system: true },
    { name: 'auditor', label: 'مدقق', is_system: true },
  ];
  const roles = {};
  for (const r of roleDefs) { roles[r.name] = await prisma.role.upsert({ where: { name: r.name }, update: { label: r.label }, create: r }); }
  console.log(` الأدوار: ${Object.keys(roles).length}`);
  ...
  const permissions = {};
  for (const p of permissionDefs) { permissions[p.key] = await prisma.permission.upsert({ where: { key: p.key }, update: { label: p.label }, create: p }); }
  console.log(` الصلاحيات: ${Object.keys(permissions).length}`);

  const allPerms = Object.keys(permissions);
  const rolePermMap = {
    super_admin: allPerms,
    admin: allPerms.filter(k => k !== 'admin:manage_permissions'),
    manager: ['field:view_product_cost','field:view_profit_margin','field:view_profit_amount','field:view_inventory_value','field:view_daily_profit','field:view_monthly_profit','field:view_safe_balance','field:view_financial_summary','field:view_customer_balance','field:view_supplier_balance','field:view_purchase_costs','dashboard:view_executive_dashboard','dashboard:view_company_profit','dashboard:view_branch_profit','dashboard:view_inventory_value','dashboard:view_financial_summary','financial:view_profit_reports','financial:view_financial_reports','financial:view_cost_reports','inventory:manage','inventory:adjustment','inventory:transfer','inventory:count','inventory:wastage','sales:create','sales:edit','sales:cancel','sales:hold','sales:resume','returns:create','returns:cancel','products:manage','products:manage_categories','business:manage_suppliers','business:manage_customers','business:manage_purchases','business:manage_expenses','reporting:view_reports','reporting:export_reports','expense:view','expense:create','expense:edit','expense:approve','expense:category:manage','report:financial','report:export','shift:open','shift:close','shift:approve','cash_register:manage','currency:exchange','template:manage','sale:cancel:review','inventory:movement:view','inventory:valuation:view','inventory:low_stock:view'],
    accountant: ['field:view_product_cost','field:view_profit_margin','field:view_profit_amount','field:view_inventory_value','field:view_daily_profit','field:view_monthly_profit','field:view_safe_balance','field:view_financial_summary','field:view_customer_balance','field:view_supplier_balance','field:view_purchase_costs','dashboard:view_executive_dashboard','dashboard:view_company_profit','dashboard:view_financial_summary','financial:view_profit_reports','financial:view_financial_reports','financial:view_cost_reports','expense:view','expense:create','report:financial','report:export','reporting:view_reports','reporting:export_reports'],
    inventory_manager: ['dashboard:view_executive_dashboard','dashboard:view_inventory_value','inventory:manage','inventory:adjustment','inventory:transfer','inventory:count','inventory:wastage','products:manage','inventory:movement:view','inventory:valuation:view','inventory:low_stock:view'],
    cashier: ['sales:create','sales:cancel','sales:hold','sales:resume','returns:create','expense:view','shift:open','shift:close'],
    auditor: ['financial:view_profit_reports','financial:view_financial_reports','financial:view_cost_reports','reporting:view_reports','reporting:export_reports','audit:view_logs','report:financial','report:export','inventory:movement:view','inventory:valuation:view','inventory:low_stock:view'],
  };
  ...
  for (const [roleName, perms] of Object.entries(rolePermMap)) {
    ...
    for (const permKey of perms) {
      ...
      if (!permId) continue;
      await prisma.rolePermission.upsert({ where: { role_id_permission_id: { role_id: roleId, permission_id: permId } }, update: {}, create: { role_id: roleId, permission_id: permId } });
      mappingCount++;
    }
  }
  console.log(` تعيينات الصلاحيات: ${mappingCount}`);
  ...
  for (const u of seedUsers) {
    ...
    if (!existing) {
      await prisma.user.create({ data: { branch_id: BRANCH_ID, name: u.name, username: u.username, password_hash: hash, role: u.role, role_id: roles[u.role]?.id, is_active: true } });
    } else {
      await prisma.user.update({ where: { username: u.username }, data: { password_hash: hash, role_id: roles[u.role]?.id, is_active: true } });
    }
  }
  await prisma.user.update({ where: { username: 'admin' }, data: { role_id: roles.admin.id } });
  console.log(` المستخدمين: ${seedUsers.length + 1}`);
